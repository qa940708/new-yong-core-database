...
(() => {
  ...
  function render() {
    ...
    const v=s.video;
    if(v.status==='available'&&v.url){const url=new URL(v.url,base);if(url.protocol==='https:'||url.origin===location.origin)video=`<video controls playsinline preload="metadata"${v.poster?` poster="${esc(new URL(v.poster,base).href)}"`:''} aria-label="${esc(dept.name+' '+stage.name+' '+s.name+' 實機施放')}" src="${esc(url.href)}">此瀏覽器不支援影片播放。</video><p>${esc(dept.name)}・${esc(stage.name)}實機展示</p>`;}
  ...
  async function boot() {
    try {
      ...
      fromUrl();guide();render();$('updated').textContent='奧義分層預覽 v2｜資料整理 '+data.updated;
